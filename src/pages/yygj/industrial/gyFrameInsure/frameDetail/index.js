    ...
    if(this.props.location.status == '待确认'){
      buttons = (
        <div style={{textAlign:'center'}}>
          <Link to={"/yygj/industrial/gyFrameInsure"}><Button  onClick={ok} type={"primary"} style={{marginRight:'20px'}}>确认</Button></Link>
          <Link  to={"/yygj/industrial/gyFrameInsure"}><Button type={"primary"}>关闭</Button></Link>
        </div>
      )
    }else{
      buttons = (
